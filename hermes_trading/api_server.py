...
def get_api_key() -> str:
 """Get the API key from environment or generate a default."""
 return os.getenv("API_KEY", "hermes-secret-key-default")

def verify_api_key(credentials: HTTPAuthCredentials = Depends(security)) -> bool:
...
@app.get("/api/data")
async def get_data(verified: bool = Depends(verify_api_key)):
